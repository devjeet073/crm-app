...
import { ArrowLeft, Shield, Users, Pencil, Trash2, UserMinus, X } from 'lucide-react';
...
export default function RoleShow({ role, users, crmModules, crudActions }: PageProps) {
    ...
    function handleAssignUser(e: React.FormEvent) {
        ...
        router.post(rolesAssignUser.url(role), { user_ids: selectedUsers.map(u => u.id) }, { preserveScroll: true, onSuccess: () => setSelectedUsers([]) });
    }
            ...
            <div className="flex flex-1 flex-col gap-6 p-4">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" onClick={() => router.visit(rolesIndex.url())}>
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                <Users className="h-4 w-4" /> Users ({role.users_count})
                            </CardTitle>
                            <CardDescription>Users directly assigned this role</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            <form onSubmit={handleAssignUser} className="flex gap-2 items-start">
                                <UserMultiSelect
                                    selectedUsers={selectedUsers}
                                    onChange={setSelectedUsers as any}
                                />
                                <Button type="submit" size="sm" disabled={selectedUsers.length === 0}>Assign</Button>
                            </form>
                            <Separator />
                            <WhenVisible data="users" fallback={<div className="text-sm text-muted-foreground py-2 animate-pulse">Loading users...</div>}>
