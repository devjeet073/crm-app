...
export default function DocumentFolderEdit({ folder, parentFolders }: PageProps) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Documents', href: '/documents' },
        { title: 'Folders', href: '/document-folders' },
        { title: `Edit: ${folder.name ?? folder.id}`, href: `/document-folders/${folder.id}/edit` },
    ];
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        put(`/document-folders/${folder.id}`);
    }

    return (
        <>
            <Head title={`Edit Folder – ${folder.name ?? folder.id}`} />

            <div className="flex flex-1 flex-col gap-6 p-4">
                <Heading
                    title={`Edit Folder: ${folder.name ?? folder.id}`}
                    description="Update folder details."
                />

                <form onSubmit={handleSubmit} className="max-w-lg space-y-6">
                    <div className="space-y-1.5">
                        <Label htmlFor="folder-name">Name <span className="text-destructive">*</span></Label>
                        <Input
                            id="folder-name"
                            value={data.name}
...
DocumentFolderEdit.layout = ({ children }: { children: React.ReactNode }) => children;
